...
[GCC](https://gcc.gnu.org) testsuite for AVR and parts of the
...
* The `.dejagnurc` file sets the `avrlibc_include_dir` variable that points
  ...
  from AVR-LibC.  Adjust this path to point to the `include/avr` subdir
...
The timing will also depend slightly on which instructions are being
simulated.
...
The values of all the General Purpose Registers can be printed with:
...
AVRtest supports syscalls `avrtest_<f>` with the obvious semantics for
`<f>` in: `printf`, `fprintf`, `sprintf`, `snprintf`, `vprintf`, `vfprintf`,
`vsprintf`, `vsnprintf`, `printf_P`, `fprintf_P`, `sprintf_P`, `snprintf_P`,
`vprintf_P`, `vfprintf_P`, `vsprintf_P`, `vsnprintf_P`.

The emulations try to mimic the [AVR-LibC behavior][1], including the `%S`
format for strings in program memory, though more formats are supported like
`long long` and IEEE double.  The formats in the format string refer to
the ABI of the target, e.g. `%d` refers to a 16-bit integer.
Variable field width and precision like in `%*d` is not supported.

To use these functions, `<stdio.h>` must be included prior to avrtest.h.
The functions that take a `FILE*` argument must get `stdout` or `stderr`.
The passed output stream is used to indicate where the output should
go (the host's stdout or stderr stream), i.e. the stream's `put()` callback
is ignored.

[1]: https://avrdudes.github.io/avr-libc/avr-libc-user-manual/group__avr__stdio.html#gaa3b98c0d17b35642c0f3e4649092b9f1
...
* To date (2026), Clang has bugs so that programs that use `avrtest.h`
  cannot be compiled due to internal compiler errors.
...
If you want to find out what value lead to the round with minimal or maximal
